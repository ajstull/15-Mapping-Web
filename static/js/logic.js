...
//cariable to hole tectonic plates layer
...
.then(function(plateData){
    ...
    L.geoJson(plateData,{
        // add stlying to make lines visible
        color: "yellow",
        weight: 1
...
// get the data fro the earthquakes and populate the layergroup
//call the USGS GeoJson API
d3.json("https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/all_week.geojson")
.then(
    function(earthquakeData){
        // console log to make sure data loaded
        // plot circles, where radius is dependent on magnitude and color is dependent on depth
       
       //make a funct that chooses the cokor of the data point
        ...
        // make a funct that determines the size of the radius
        function radiusSize(mag){
            if (mag == 0)
                return 1; //make sure that a 0 mag earthquake actually shows up
            else
                return mag * 5; //make sure that the cirle is pronounced into the map
        }
        ...
        L.geoJson(earthquakeData,{
            // make each feature a marker that is on the map, each marker is a circleddd
...
legend.onAdd = function() {
    ...
    // set the colors for the intervals taken from dataColor Funct line 81
    var colors = [
        "green",
        "#cafc03",
        "#fcad03",
    ...
    // loop through the intervals and the colors and generate the label 
    // with coloured square for each interval
